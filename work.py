...
class Student:
    ...
    def __str__(self):
        return f"Имя: {self.name}\nФамилия: {self.surname}\nСредняя оценка за домашние задания: {round(average_rating(self), 2)}\nКурсы в процессе изучения: {self.courses_in_progress}\nЗавершенные курсы: {self.finished_courses}"
...
class Lecturer(Mentor):
    ...
    def __str__(self):
        return f"Имя: {self.name}\nФамилия: {self.surname}\nСредняя оценка за лекции: {round(average_rating(self), 2)}"
...
print(ruoy == clark)
print()
print('Средняя оценка за домашние задания по всем студентам -', course_average_grade(get_list_students()))
print('Средняя оценка за лекции всех лекторов -', course_average_grade(get_list_lectors()))
